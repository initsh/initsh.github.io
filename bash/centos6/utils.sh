...

{
    ...
    then
        yum -y install traceroute 2>&1
    fi
    
    if ! rpm --quiet -q nmap
    then
        yum -y install nmap 2>&1
    fi
    
    if ! rpm --quiet -q nmap-ncat
    then
        yum -y install nmap-ncat 2>&1
    fi
    
    if ! rpm --quiet -q tcpdump
    then
        yum -y install tcpdump 2>&1
    fi
    
    if ! rpm --quiet -q mailx
    then
        yum -y install mailx 2>&1
    fi
    
    if ! rpm --quiet -q lsof
    then
        yum -y install lsof 2>&1
    fi
    
    if ! rpm --quiet -q parted
    then
        yum -y install parted 2>&1
    fi
    
    if ! rpm --quiet -q openssh-clients
    then
        yum -y install openssh-clients 2>&1
    fi
    ...
    if ! rpm --quiet -q zip
    then
        yum -y install zip 2>&1
    fi
    
    if ! rpm --quiet -q unzip
    then
        yum -y install unzip 2>&1
    fi
    
    if ! rpm --quiet -q vim-enhanced
    then
        yum -y install vim-enhanced 2>&1
    fi
    
    # if not installed (return 127), download jq
    if [ "$(jq --help >/dev/null 2>&1; echo $?)" -eq 127 ]
    then
        v_jq_url="https://github.com/stedolan/jq/releases/download/jq-1.5/jq-linux64"
        echo "bash# curl ${v_jq_url} -o /usr/bin/jq"
        curl -LRs "${v_jq_url}" -o /usr/bin/jq && chmod 755 /usr/bin/jq 
        ls -dl /usr/bin/jq*
        if [ "$(jq --help >/dev/null 2>&1; echo $?)" -eq 127 ]
        then
